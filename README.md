...
**1. User Side (Edge Layer)**: 
Handles microphone capture, frame processing, RNNoise denoising, and Voice Activity Detection (VAD) gating. Its job is to ensure only clean speech is sent to the network, discarding background noise.Cloud Side (Processing Layer): Handles the heavy compute of Speech-to-Text and Intent Recognition.Developer Side (Control Layer): Enforces confidence thresholds to route the query safely (Accept, Confirm, or Reject).

**2. Cloud Side (Processing Layer)**: 
Handles the heavy compute of Speech-to-Text and Intent Recognition.

**3. Developer Side (Control Layer)**:
Enforces confidence thresholds to route the query safely (Accept, Confirm, or Reject).

**How we tailored the POC:**
...
graph TD
    ...
    subgraph TransitLayer [Data in Transit]
        I -- TLS 1.3 / HTTPS --> J[FastAPI Backend]
    end
    ...
    Q --> T([UI: Mother-Tongue Confirmation Prompt])
...
6. Test the UI:
```
Open your browser and navigate to http://localhost:3000/new_index.html to test the Voice Chatbot.
```
